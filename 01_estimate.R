...
file<-"code_SFEMQRE/01_estimate_SFEMQRE.rds"

if (!file.exists(file) | RERUN) {
  
    # Estimate QRE-SFEM model
  ...
  # Estimate vanilla SFEM model

  file<-"code_SFEMQRE/01_estimate_SFEM.rds"
  ...
  LOO |>
    saveRDS("code_SFEMQRE/01_estimate_loo.rds")
  
  (postprobs<-post_prob(bs.SFEMQRE,bs.SFEM) |>
    data.frame()
  )
  
  postprobs |>
    saveRDS("code_SFEMQRE/01_estimate_postprobs.rds")
  ...
  LOGML |>
    saveRDS("code_SFEMQRE/01_estimate_logml.rds")
